...
module.exports = function (app,io){
    ...
    //     User.findOne({"handle":req.body.handle},function(err,doc){
    //         if(err){
    //             res.json(err); 
    //         }
    //         if(doc == null){
...
//         res.setHeader("Access-Control-Allow-Method","'GET, POST, OPTIONS, PUT, PATCH, DELETE'");
//         handle = req.body.handle;
//         User.findOne({"handle":req.body.handle, "password":req.body.password},function(err,doc){
//             if(err){
//                 res.send(err); 
//             }
//             if(doc==null){
...
//     });
    ...
    io.on('connection',function(socket){
        ...
        User.find({"handle" : handle},{friends:1,_id:0},function(err,doc){
            if(err){res.json(err);}
            else{
                friends=[];
                pending=[];
    ...
    app.post('/friend_request',function(req,res){
        ...
        res.setHeader("Access-Control-Allow-Method","'GET, POST, OPTIONS, PUT, PATCH, DELETE'");
        friend=true;
        User.find({"handle" : req.body.my_handle,"friends.name":req.body.friend_handle},function(err,doc){
            if(err){res.json(err);}
